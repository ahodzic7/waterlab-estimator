...
% X_MPC = zeros((Nxt+ Nxp),N); 
% U_MPC = casadi.DM.zeros(Nxt,N);
% S_MPC = casadi.DM.zeros(Nxt,N);
% Y_MPC = casadi.DM.zeros((Nxt-1),N);

Y     = P(2)*g(X(Nxt+ Nxp,:),P(3));

%% =========================================== Objective function ==============================
% 5000*sumsqr(Gs.*S) + 0.01*sumsqr(Gx.*X(1:Nxt,:)) + 50*sumsqr(Gu.*U);  
objective = sumsqr(U) + sumsqr(X(1:Nxt,:) - X_ref) + sumsqr(S);                          
...
%% ====================================== Solver settings ==================================
opti.set_initial(X, 0.0001);                                                    % initial system state 
opti.set_initial(S, 0.0001);                                                    % initial system slack 
opti.set_initial(U, 0);                                                         % initial decision variable
...
if warmStartEnabler == 1                                                        % Parametrized Open Loop Control problem with WARM START
    OCP = opti.to_function('OCP',{X0,D,P,opti.lam_g,opti.x,DT},{U(:,1),S(:,1),Y(:,1),opti.lam_g,opti.x},{'x0','d','p','lam_g','x_init','dt'},{'u_opt','s_opt','y_opt','lam_g','x_init'});
elseif warmStartEnabler == 0                                                    % Parametrized Open Loop Control problem without WARM START 
    OCP = opti.to_function('OCP',{X0,D,P,DT},{U(:,1),S(:,1),Y(:,1)},{'x0','d','p','dt'},{'u_opt','s_opt','y_opt'});
end
