...
sigma_X_sim(:,1) = 0; 
for i = 1:Hp
    sigma_X_sim(:,i+1) = sigma_X_sim(:,i) + 0.001;
end
...
elseif warmStartEnabler == 0
    ...
    OCP = opti.to_function('OCP',{X0,D,T,R,sigma_D,sigma_X(:,:)},{U(:,:),S_of(:,:),S_uf(:,:)},{'x0','d','dt','r','sigma_d','sigma_x'},{'u_opt','s_of_opt','s_uf_opt'});
end
...
%     hold on;
    elseif warmStartEnabler == 0
    % Parametrized Open Loop Control problem without WARM START 
    [U_sim_single_step, S_of_sim_single_step,S_uf_sim_single_step] = (OCP(X_sim(:,i), disturbance_mean(:,i:i+Hp-1), dt_sim, R_sim, sigma_D_sim, sigma_X_sim));
    end
...
%     end
    ...
    if plotting
        ...
        hold on;
        stairs(i-1:1:Hp-2+i,full(U_sim_single_step(:,1:end)),'g');
        hold on;
...
ylabel('$\bar{V}_k$','Interpreter','latex');
