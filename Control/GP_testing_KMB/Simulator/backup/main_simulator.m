...
N = 2600;                                                                                % length of simulation (dependent on the length of disturbance data)
...
for i = 1:1:N                                                       
    ...
    elseif controlType == 2    
        [U_MPC,S_MPC,Y_MPC,lam_g,x_init] = OCP(X_sim(:,i), D_sim(:,(i)*(20)-19:20:(i-1)*20 + (Hp)*20-19), P_sim, X_ref_sim(:,1 + (i-1)*t_resample), lam_g, x_init, dt_sim);
        U_opt(:,i) = full(U_MPC);
    end
