...
if warmStartEnabler == 1
    ...
    OCP = opti.to_function('OCP',{X0,D,opti.lam_g,opti.x,T,R,sigma_D},{U(:,1),S(:,1),opti.lam_g,opti.x,sigma_X(:,:)},{'x0','d','lam_g','x_init','dt','r','sigma_d'},{'u_opt','s_opt','lam_g','x_init','sigma_x'});
elseif warmStartEnabler == 0
    % Parametrized Open Loop Control problem without WARM START 
    OCP = opti.to_function('OCP',{X0,D,T,R,sigma_D},{U(:,1),S(:,1),sigma_X(:,:)},{'x0','d','dt','r','sigma_d'},{'u_opt','s_opt','sigma_x'});
end
...
for i = 1:1:N
    ...
    if warmStartEnabler == 1
    % Parametrized Open Loop Control problem with WARM START
    [U_sim(:,i), S_sim(:,i), lam_g, x_init, sigma_X_sim(i,:)] = (OCP(X_sim(:,i), disturbance_mean(:,i:i+Hp-1), lam_g, x_init, dt_sim, R_sim, sigma_D_sim));
    elseif warmStartEnabler == 0
    % Parametrized Open Loop Control problem without WARM START 
    [U_sim(:,i), S_sim(:,i), sigma_X_sim(i,:)] = (OCP(X_sim(:,i), disturbance_mean(:,i:i+Hp-1), dt_sim, R_sim, sigma_D_sim));
    end
    % Simulate dynamics
