    ...
    subplot(5,1,4)
    title('CC-Slack Variables');
    hold on
    plot(elapsed_time, S_ub_sim_num(1,1:step),'b')
    hold on
    plot(elapsed_time, S_ub_sim_num(2,1:step),'g')
    hold on
    for i = 1:2
    plot(elapsed_time(end), S_ub_sim_num(i,step),'r*');
    hold on
    end
    plot(future_time ,[S_sim_num(1,step),S_ub_out_num(1,:)],'b--');
    hold on
    plot(future_time ,[S_sim_num(2,step),S_ub_out_num(2,:)],'g--');
    hold off
    xlim([0,N+Hp])
